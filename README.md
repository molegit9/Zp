...
1. **Chrome 확장 프로그램 분기:** 사용자가 웹페이지 내의 링크에 마우스를 올리거나 텍스트를 드래그하면 `content.js`가 이를 가로채어 백엔드 서버(`/api/v1/analyze` 또는 `/api/v1/analyze/text`)로 전송합니다.
2. **백엔드 분석 흐름 (FastAPI + Redis + SQLite + ChromaDB):** 
   - **캐시 및 Fast Pipeline:** 빠른 응답 속도를 위해 이전에 검사된 결과가 DB에 있거나 최상위 공식 도메인(구글, 네이버 등)인 경우 즉시 결과를 반환합니다.
   - **실시간 스트리밍 (NDJSON):** 서버는 분석 과정(VirusTotal 스캔, LLM 처리 등)이 시작될 때마다 그 상태를 실시간으로 프론트엔드에 스트리밍(`StreamingResponse`)하여 사용자가 현재 진행 상황을 툴팁으로 볼 수 있게 합니다.
   - **외부 연동 (VT & RDAP & RAG):** URL인 경우 VirusTotal(블랙리스트)과 RDAP(도메인 나이)를 동시 검사하며, 드래그 텍스트인 경우 로컬 벡터 DB(ChromaDB)를 통해 과거 유사 피싱 판례를 검색(RAG)합니다.
   - **2차 심층 분석 (Google Gemini API):** 수집된 메타데이터(오타 사칭, VT 결과, RAG 판례 등)를 바탕으로 Gemini API가 문맥과 의도를 파악하여 최종 안전도 점수(0~100)를 산출합니다.
3. **분석 결과 시각화 및 제어:** 익스텐션은 실시간 진행 상황을 보여주다가 최종적으로 안전(초록), 주의(노랑), 위험(빨강) 등의 시각적 표시와 상세 이유를 툴팁으로 렌더링합니다. (익스텐션 팝업 아이콘을 클릭해 실시간 진행상황 툴팁 표시 여부를 켜고 끌 수 있습니다.)
